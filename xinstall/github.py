...
from .utils import (option_version, option_python, option_pip_bundle,
                    add_subparser, run_cmd)
from . import utils
...
def _github_download(args):
    ...
    releases = resp.json()
    assets = next(release["assets"] for release in releases
                  if parse(release["tag_name"]) in spec)
    # get download URL
    if args.keyword:
        filter_ = lambda name: all(kwd in name for kwd in args.keyword)
    else:
        filter_ = lambda name: True
    url = next(asset["browser_download_url"] for asset in assets
               if filter_(asset["name"]))
    # download the assert
...
def _github_args(subparser):
    ...
    option_version(
        subparser,
        help=
        "The version specifier of the package to download/install/configure.")
    subparser.add_argument(
        "-k",
        "--kwd",
        "--keyword",
...
def install_py_github(args) -> None:
    ...
    """
    utils.install_py_github(url=args.url,
                            user=args.user,
                            pip=args.pip,
