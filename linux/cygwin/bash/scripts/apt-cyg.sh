...
function usage() {
    ...
    echo "  \"apt-cyg search <patterns>\" to search packages matching patterns"
...
function findworkspace() {
    ...
    if test -e /etc/setup/last-cache; then
        tmp="`head -1 /etc/setup/last-cache`"
        cache="`cygpath -au "$tmp"`"
    fi

    if test -e /etc/setup/last-mirror; then
...
while test $# -gt 0; do
    case "$1" in
        ...
        update|list|search|show|packageof|install|remove)
